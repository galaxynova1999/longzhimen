<template>
  <div>
    <div id="u0">
      <div id="u257" class="二级标题">
        ...
        <div id="u258_text" class="text">
          <p>
            <Icon type="md-create" />
            <span style="text-decoration:none;">单元测试</span>
          </p>
        </div>
        <div id="u3_img">
    ...
    <div id="question" v-if="unit_test_question.length!=0">
      <p>{{current_page+1}}.{{unit_test_question[current_page].exercise}}</p>
    </div>
    <div id="option" v-if="unit_test_question.length!=0">
      <Button
        :class="btn_type1"
        long
        shape="circle"
        size="large"
        @click="OP_A"
      >{{unit_test_question[current_page].option_a}}</Button>
      <Button
        :class="btn_type2"
        ...
        size="large"
        @click="OP_B"
      >{{unit_test_question[current_page].option_b}}</Button>
      <Button
        :class="btn_type3"
        style="margin-top:15px;"
        long
        ...
        size="large"
        @click="OP_C"
      >{{unit_test_question[current_page].option_c}}</Button>
      <Button
        :class="btn_type4"
        style="margin-top:15px;"
        long
        ...
        size="large"
        @click="OP_D"
      >{{unit_test_question[current_page].option_d}}</Button>
    </div>
...
export default {
      ...
      if (this.current_page + 1 === this.page_total) {
        ...
        for (let i = 0; i < this.current_answer.length; i++) {
          if (this.current_answer[i] === this.right_answer[i]) {
            ...
            score += 28;
            this.arr.push({
              wordid: this.unit_test_question[this.current_page].word_id,
              right: 1
            });
          } else {
            this.arr.push({
        ...
        //TODO 答题完成跳转
        let that = this;
        this.$Message.success("您单元测试获得了" + score + "分");
        ...
        params.append("unitstr", this.unit);
        params.append("scorestr", score);
        axios.post("addUnittestScore.do", params).then(function(res) {
          if (res.data === "0") {
  ...
  created() {
    ...
    axios.post("unittest.do", params).then(function(res) {
      that.unit_test_question = res.data;
      //保存答案
      that.page_total = that.unit_test_question.length;
      for (let i = 0; i < that.unit_test_question.length; i++) {
        that.right_answer.push(that.unit_test_question[i].answer);
      }
    });
  }
};
